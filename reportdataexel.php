...
while($row = mysqli_fetch_array($query))
{
	...
	$sheet->setCellValue('F'.$i, $row['temlahir']);

	$sheet->setCellValue('G'.$i, $row['tgllahir']);

	$sheet->setCellValue('H'.$i, $row['noregakte']);
	...
	$sheet->setCellValue('V'.$i, $row['temtinggal']);

	$sheet->setCellValue('W'.$i, $row['transport']);
...
$writer->save('Data Pendaftaran.xlsx');
